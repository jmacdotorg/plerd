...
To publish a post, simply move it to Plerd's source directory. Plerd will, once it notices the new file, change the file in two ways:

* It will give the file a timestamp recording the date and time of its publication. This will appear in its own line, after the title line. (Plerd won't do this if, for whatever reason, you already provided a timestamp in W3C format.)

* It will rename the file from your working title to Plerd's own source filename format, comprising the publication date followed by the title. (Plerd won't do this if you already named the file in this format, _and_ the date in the filename agrees with the post's publication date.)

(Note that, depending upon various factors like your text editor's features and whether you're using Plerd with Dropbox, you may need to close the file in your editor and re-open it at its new name and location in order to work with it further.)
